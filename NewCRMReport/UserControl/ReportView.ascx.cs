...
{
    public partial class ReportView_ascx : System.Web.UI.UserControl
    {
        ...
        {
            /*
             * To Word  option reference to "WORDOPENXML" / "WORD"
             * To Excel option reference to "EXCELOPENXML" / "Excel"
             * To PDF   option reference to "PDF"
             * To Tiff  option reference to "IMAGE"
             */
            ExportFormat(ReportViewerMaster, "WORDOPENXML", false);
            ExportFormat(ReportViewerMaster, "IMAGE", true);
        }

        public void ExportFormat(ReportViewer ReportViewerID, string strFormatName, bool Enable)
        {
            FieldInfo info;
            foreach (RenderingExtension extension in ReportViewerID.LocalReport.ListRenderingExtensions())
            {
                if (extension.Name == strFormatName)
                {
                    ...
                    info.SetValue(extension, Enable);
                    break;
                }
            }
        }
        ...
        public void DataBind(DataSet ds)
        {
            ...
            headerTxt[0] = new ReportTextBoxControl() { Name = "txtReportTitle", ValueOrExpression = new string[] { ReportTitle } };
        ...
        static string GenerateTextBox(string strControlIDPrefix, string strName, string strValueOrExpression = "", bool isFieldValue = true, ReportDimensions padding = null)
        {
            ...
            strTextBox = @" <Textbox Name=""" + strControlIDPrefix + strName + @"""> 
                    ...
                    <Paragraph> 
                        <TextRuns> 
                        <TextRun>";
            strTextBox += @"<Value>=Fields!" + strName + @".Value</Value><MarkupType>HTML</MarkupType>";
            strTextBox += @"<Style /> 
                        </TextRun> 
                        </TextRuns> 
                        <Style /> 
                    ...
                    <ActionInfo>
                        <Actions>
                            <Action>
                                <Hyperlink>=Fields!" + strName + @".Value</Hyperlink>
                            </Action>
                        </Actions>
                    </ActionInfo>
                    <Style> 
